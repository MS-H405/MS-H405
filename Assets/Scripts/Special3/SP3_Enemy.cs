...
public class SP3_Enemy : MonoBehaviour
{
	#region 定数
	...
	[SerializeField]	GameObject MechaPieroObj;
...
//		}
		...
		Vector3 vStartScale = MechaPieroObj.transform.localScale;
		Vector3 vSpeed = CON_FLY_SPEED;

		for (float fTime = 0.0f; fTime < CON_DELETE_TIME; fTime += Time.deltaTime)
		{
			vSpeed.y += CON_FLY_ACCELE_Y * Time.deltaTime;			// 移動
			MechaPieroObj.transform.position += vSpeed * Time.deltaTime;

			MechaPieroObj.transform.Rotate(CON_FLY_ROTATE * Time.deltaTime);		// 回転

			MechaPieroObj.transform.localScale = Vector3.Lerp(vStartScale, Vector3.zero, fTime / CON_DELETE_TIME);		// 拡大率
	...
	public void Stan()
	{
		Animator animator = MechaPieroObj.GetComponent<Animator>();
