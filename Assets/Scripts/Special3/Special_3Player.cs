...
public class Special_3Player : MonoBehaviour
{
	#region 定数

	const float CON_WAIT_BACKJAMP = 0.5f;		// 後方ジャンプするまでの待ち時間(モーションと併せる)
	const float CON_TIME_BACKJAMP = 2.0f;		// 後方ジャンプしている時間
	const float CON_TIME_CHANGE_BAGPIPE = 1.0f;	// 後方ジャンプ中にプレイヤーモデルを入れ替えるタイミング
...
//	const float CON_RIDEBALL_ACCELE_Y = -20.0f;	// 玉に乗るときの加速度Y
	...
	bool bChangeBagpipe = true;	// バグパイプモデルに入れ替えるかどうか

	// Effekseer関係
	[SerializeField] GameObject SetEffekseerObj;
	SetEffekseerObject cs_SetEffekseerObject;

	Special_3PlayerMotion cs_Special_3PlayerMotion;
	//Vector3 vStartEular;
	//Vector3 vEndEular;
	...
	void Start ()
	{
		NormalPlayerObj.SetActive(true);	// ノーマルプレイヤーは最初は表示
		BagpipePlayerObj.SetActive(false);	// バグパイププレイヤーは最初非表示
		...
		cs_Special_3PlayerMotion = GetComponent<Special_3PlayerMotion>();
	}
	...
	public bool BackJamp()
	{
		...
		if (bInit)
		{
			...
			cs_Special_3PlayerMotion.Backjamp_Start();	// 後方ジャンプモーション
			//vStartEular = new Vector3(180.0f, 0.0f, 0.0f);
			//vEndEular = new Vector3(540.0f, 0.0f, 0.0f);

			//MovieSoundManager.Instance.PlaySE(MovieSoundManager.eSeValue.SP_Jump);		// ジャンプ音
		...
		if (fTime > CON_TIME_BACKJAMP)
		{
			...
			MovieSoundManager.Instance.PlaySE(MovieSoundManager.eSeValue.SP_RideOn);	// 着地音
		...
		// プレイヤーモデルの切り替え
		if(fTime >= CON_TIME_CHANGE_BAGPIPE && bChangeBagpipe)
		{
			NormalPlayerObj.SetActive(false);
			BagpipePlayerObj.SetActive(true);

			bChangeBagpipe = false;
		}
	...
	public void ParentConf()
	{
		...
		NormalPlayerObj.SetActive(false);	// ノーマルプレイヤーは最初は表示
		BagpipePlayerObj.SetActive(true);	// バグパイププレイヤーは最初非表示
	}
