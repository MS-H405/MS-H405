...
public class Special_3Camera : MonoBehaviour
{
	...
	const float CON_BAGPIPEFIRE_WAIT = 1.5f;		// バグパイプから出る火の玉の座標を見始めるまでの待機時間
	const float CON_BAGPIPEFIRE_TIME = 1.3f;		// バグパイプから出る火の玉の座標を見ている時間

	const float CON_BALLIGNITION_TIME = 1.0f;		// カメラが玉着火までいってから、エフェクトが出るまでの時間
	...
	[SerializeField]	GameObject SP_BagpipeObj;
	Vector3 vLookAt3;			// バグパイプから出る火の玉を見るときに使う注視点
	bool bIgnition = true;
	...
	public bool LookAtFire()
	{
		...
		fWait += Time.deltaTime;
		if (fWait < CON_BAGPIPEFIRE_WAIT)
			return false;

		fTime += Time.deltaTime;
		if(fTime >= CON_BAGPIPEFIRE_TIME)
		{
			bInit = true;
			return true;
		}

		vLookAt3 = Vector3.Lerp(vLookAt3, SP_BagpipeObj.transform.position, 0.1f);
		transform.LookAt(vLookAt3);
	...
	public bool CenterLine()
	{
		if (bInit)
		{
			fTime = 0.0f;
			fWait = 0.0f;
			EulerMove(CameraMoveList[14], CameraMoveList[15], 0.0f);

			bInit = false;
		}

		fWait += Time.deltaTime;
		if (fWait < CameraMoveList[14].fWait)
			return false;

		fTime += Time.deltaTime / CameraMoveList[14].fTime;
		if (fTime > 1.0f)
		{
			EulerMove(CameraMoveList[14], CameraMoveList[15], 1.0f);
		...
		EulerMove(CameraMoveList[14], CameraMoveList[15], fTime);

		return false;
	}

	// プレイヤーの玉に着火
	public bool BallIgnition()
	{
		if (bInit)
		{
			fTime = 0.0f;
		...
		fWait += Time.deltaTime;
		if (fWait < CameraMoveList[16].fWait)
			return false;

		transform.position = CameraMoveList[16].vPos;
		transform.eulerAngles = CameraMoveList[16].vEuler;

		fTime += Time.deltaTime;
		if (fTime > CameraMoveList[16].fTime)
		{
			fTime = 0.0f;
			fWait = 0.0f;
			bInit = true;
		...
		// エフェクト表示
		if(fTime >= CON_BALLIGNITION_TIME && bIgnition)
		{
			cs_SetEffekseerObject.NewEffect(14);	// 玉着火エフェクト
			bIgnition = false;
		}
	...
	private void SetPosEulerTime()
	{
		...
		// 14
		temp.vPos = new Vector3(10000.0f, 12.2f, -105.1f);		// CnterLine開始
		temp.vLook = new Vector3(10000.0f, 6.7f, -96.8f);
		temp.vEuler = new Vector3(33.6f, 0.0f, 0.0f);
		temp.fTime = 1.6f;
		temp.fWait = 1.8f;
		CameraMoveList.Add(temp);

		// 15
		temp.vPos = new Vector3(10000.0f, 12.2f, -105.1f);		// CenterLine終了
		temp.vLook = new Vector3(10000.0f, 7.7f, -96.2f);
		temp.vEuler = new Vector3(27.0f, 0.0f, 0.0f);
		temp.fTime = 0.0f;
		temp.fWait = 0.0f;
		CameraMoveList.Add(temp);

		// 16
		temp.vPos = new Vector3(10000.0f, 6.4f, -89.2f);		// プレイヤーの玉に着火
		temp.vLook = new Vector3(10000.0f, 4.2f, -98.9f);
		temp.vEuler = new Vector3(12.9f, 180.0f, 0.0f);
		temp.fTime = 2.0f;
		temp.fWait = 0.3f;
		CameraMoveList.Add(temp);
	}
